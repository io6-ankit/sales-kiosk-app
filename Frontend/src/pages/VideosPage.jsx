...
//     const { setActiveVideo, showNotification, role } = useKiosk();
...
//             if (lastMessage.type && lastMessage.type.startsWith("VIDEO_")) {
//                 console.log('Real time video page any changes detected, Aut-fetch fresh data...')
//                 fetchVideos()
//             }
//         }
...
//         e.stopPropagation();
//         setEditItem(item);
//         setFormData({ title: item.title || '', url: item.url || item.videoUrl || '', description: item.description || '' });
//         setModalOpen(true);
//     };
...
//                                     }}
//                                 >
//                                     <PlayCircleOutlinedIcon sx={{ fontSize: 64, color: '#1976d2' }} />
//                                 </Box>
//                                 <CardContent sx={{ pb: 1 }}>
...
export const VideosPage = () => {
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 6 }}>
      ...
      {loading ? (
        ...
        <Grid container spacing={3}>
          {filteredVideoItems.map((item) => {
            ...
            // Check if user provided an explicit cover image property, or fallback to auto-first-frame
      ...
      {/* Synchronized Video Player Lightbox Dialog (Unchanged) */}
      <Dialog
