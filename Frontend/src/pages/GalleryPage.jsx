...
//     // 🔴 FIX 1: Listen to WebSocket CRUD Updates in Real-Time
//     useEffect(() => {
//         if (lastMessage) {
//             // const eventType = lastMessage.type || "";
//             // if (eventType.includes) 'GALLERY'
//             if (lastMessage.type && lastMessage.type.startsWith("GALLERY_")) {
//                 console.log('Real time gallary page any changes detected, Aut-fetch fresh data...')
//                 fetchGallery()
//             }
//         }
...
//         e.stopPropagation();
//         setEditItem(item);
//         setFormData({ title: item.title || '', url: item.url || item.imageUrl || '', description: item.description || '' });
//         setModalOpen(true);
//     };
...
//                                         position: 'relative',
//                                         transition: '0.3s',
//                                         // 🔴 FIX 2: Highlight Active Image Card Sync for Viewer Mode
...
//             {/* 🔴 FIX 3: Fullscreen Image Preview Lightbox (Auto-Opens on Viewer when Controller Clicks) */}
//             <Dialog
...
//                 fullWidth
//             >
//                 <Box sx={{ position: 'relative', bgcolor: '#000', display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
//                     <IconButton
...
export const GalleryPage = () => {
  ...
  // Listen to WebSocket CRUD Updates in Real-Time (Unchanged)
  useEffect(() => {
  ...
  // NEW: Compute filtered list without mutating or replacing galleryItems
  ...
  const handleCardClick = (item) => {
    // Only Controller triggers image selection to mirror across screens (Unchanged)
    if (role === 'CONTROLLER') {
      setActiveImage(item);
    }
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 6 }}>
      ...
      {/* NEW: Search Bar Added cleanly above grid without disturbing other elements */}
      <Box sx={{ mb: 3 }}>
        <TextField
          fullWidth
          size="small"
      ...
      {/* Synchronized Image Lightbox Overlay Modal (Unchanged) */}
      <Dialog
        open={Boolean(activeImage)}
        onClose={handleCloseLightbox}
        maxWidth="md"
